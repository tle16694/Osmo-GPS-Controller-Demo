...
Taking the Osmo Action GPS Bluetooth controller released by DJI as a reference, this accessory can establish Bluetooth connections with devices such as DJI Osmo Action 6 / 5 Pro / 4 and Osmo 360, enabling interactive control of the camera. Users can not only send various control commands to the camera through the controller but also receive real-time camera status information, providing a smooth and convenient operation experience. Since the Osmo series cameras do not come with a built-in GPS module, the remote controller can synchronize real-time GPS data to the camera, thereby enabling the dashboard feature in the DJI Mimo app and providing users with an immersive experience.
